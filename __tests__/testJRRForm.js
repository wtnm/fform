...
describe('React Form', () => {
  ...
  it('should render all three levels', function() {
    ...
    // Note: Simply changing property 'value' will not trigger the event, we must also trigger the value _onChange event.
    likeMovieSelect.value = "yes";
    TestUtils.Simulate.change(likeMovieSelect, { value : "yes" });

    // Count amount of selects present in form
    let count = TestUtils.scryRenderedDOMComponentsWithTag(renderedForm, "select").length;
    expect(count).toEqual(3);

    // Rinse and repeat...
    let faveMovieSelect = TestUtils.scryRenderedDOMComponentsWithTag(renderedForm, "select")[2];
