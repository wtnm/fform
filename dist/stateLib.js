...
Macros.setStatus = (state, schema, UPDATABLE, item) => {
    ...
    if ((op == 'untouched' || op == 'unsubmitted') && prevVal == 0 && !selfManaged)
        return state; // stick "untouched" and "unsubmitted" to zero for elements and arrays
...
const makeDataStorage = react_ts_utils_1.memoize(function (schemaPart, oneOf, type, value = schemaPart.default) {
    ...
    if (untouched != 1)
        result.status = Object.assign(Object.assign({}, result.status), { untouched, unsubmitted: untouched });
    if (schemaPart._data$) {
        let res = [];
...
function setDirtyPROC(state, UPDATABLE, initial, current, track = []) {
    if (react_ts_utils_2.isUndefined(initial))
        initial = react_ts_utils_1.getIn(state, SymData, 'default', track);
    if (current === initial)
        return state;
    ...
    if (!schemaPart || isSelfManaged(state, track)) { //direct compare
        ...
        let keys = react_ts_utils_1.objKeys(Object.assign({}, initial, current));
        keys.forEach(key => state = setDirtyPROC(state, UPDATABLE, react_ts_utils_1.getIn(initial, key), react_ts_utils_1.getIn(current, key), track.concat(key)));
    }
    return state;
}
function updateDirtyPROC(state, UPDATABLE, initial, currentChanges, track = [], forceDirty = false) {
    const { schema } = UPDATABLE.api;
    let schemaPart;
    try {
        schemaPart = getSchemaPart(schema, track, oneOfFromState(state));
    ...
    if (!schemaPart || isSelfManaged(state, track)) { //direct compare
        ...
        forceDirty = forceDirty || !react_ts_utils_2.isMergeable(initial);
        keys.forEach(key => state = updateDirtyPROC(state, UPDATABLE, react_ts_utils_1.getIn(initial, key), react_ts_utils_1.getIn(currentChanges, key), track.concat(key), forceDirty));
    }
    return state;
}
function setPristinePROC(state, UPDATABLE, initial, track = []) {
    if (react_ts_utils_1.getIn(UPDATABLE.update, track, SymData, 'status', 'pristine')) {
        if (react_ts_utils_2.isMergeable(initial) && react_ts_utils_1.getIn(state, SymData, 'current', track) !== initial) {
            react_ts_utils_1.setIn(UPDATABLE.update, initial, SymData, 'current', track);
            react_ts_utils_1.setIn(UPDATABLE.replace, true, SymData, 'current', track);
        }
    }
    else {
        react_ts_utils_1.objKeys(react_ts_utils_1.getIn(UPDATABLE.update, track)).forEach(key => setPristinePROC(state, UPDATABLE, react_ts_utils_1.getIn(initial, key), track.concat(key)));
    }
    return state;
}
function updateState(dispatch) {
    ...
    if (prevState[SymData].initial !== state[SymData].initial) { // check dirty for initial changes
        let initialChanges = react_ts_utils_1.mergeState(prevState[SymData].initial, state[SymData].initial, { diff: true }).changes;
...
function updateCurrentPROC(state, UPDATABLE, value, replace, track = [], setOneOf) {
    if (value === SymReset || react_ts_utils_2.isUndefined(value))
        value = react_ts_utils_1.getIn(state, SymData, 'initial', track);
    ...
    if (path.length == 0 && (keyPath[0] == 'initial' || keyPath[0] == 'default')) {
        ...
        if (keyPath[0] == 'value') { // modify current
            ...
            for (let i = start; i < end; i++) {
                ...
                if (!unsubmitted)
                    react_ts_utils_1.setIn(mergeBranch[SymData], { unsubmitted: 0, submitted: true }, 'status');
                ...
                if (unsubmitted)
                    state = Macros.setStatus(state, schema, UPDATABLE, makeNUpdate(path, ['status', 'unsubmitted'], 1));
            }
            ...
            if ((oldOneOf != newOneOf) || (item.type && item.type != react_ts_utils_1.getIn(oldBranch, SymData, 'fData', 'type'))) {
                ...
                ['untouched', 'unsubmitted'].forEach(statusKey => {
                    if (react_ts_utils_1.getIn(oldBranch, SymData, 'status', statusKey) == 0)
                        branch = react_ts_utils_1.merge(branch, { [SymData]: { status: { [statusKey]: 0, [STATUS_REV[statusKey]]: true } } }); // stick untouched and unsubmitted to zero
                    ...
                    state = setDataMapInState(state, UPDATABLE, maps2enable);
                    if (react_ts_utils_1.getIn(branch, SymData, 'status', statusKey) == 0) {
