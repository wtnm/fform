...
export function fields({prefix = 'fform', blocks = ['Wrapper', 'Main', 'Title', 'Message']} = {}) {
  ...
  blocks.forEach(bl => {
    result.sets.base[bl] = {
      $_maps:
        {className: [{}, {$: '^/fn/getClassNameByProp', args: [prefix, 'fieldName'], update: 'build', replace: false}]}
    }
  });
  return result;
}
...
export function presets({prefix = 'fform', blocks = ['Wrapper', 'Main', 'Title', 'Message']} = {}) {
  ...
  blocks.forEach(bl => {
    result.sets.base[bl] = {
      $_maps:
        {className: [{$: '^/fn/getClassNameByProp', args: [prefix, 'mainPreset'], update: 'build', replace: false}, {}]}
    }
  });
  return result;
}
