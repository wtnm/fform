...
```
Подразумевается, что для сборки используется webpack или что-нибудь подобное.

Для начала, создадим форму для логина пользователя. Будем считать, что в качестве логина используется номер телефона.

Импортируем все необходимое, а так же стили из 'fform/addon/css', чтобы форма правильно отображалась и создадим схему в которой определим поля формы:
```js
import * as React from 'react';
import {render} from 'react-dom';
import {FForm, elements} from 'fform';
import 'fform/addon/css/fform.css';
...
render(<FForm core={{schema, elements}}/>, document.querySelector('#root'));
```
...
- `_placeholder` заполнитель пустого поля. Приставка `_` маркирует свойство не входящее в спецификацию JSONSchema.

Четвертое свойство `_presets` поля `password` определяет какие сеты из `elements.sets` будут использоваться для постройки поля. Если свойство `_presets` не опредено, то используется свойство `type`, но только если `type` является строкой, иначе бросится исключение. 

В корневом поле `_presets` не определено, но опрелено свойство `type  = "object"`, поэтому `ff_presets = "object"`, т.е. сет берется из `elements.sets.object`. Аналогично для поля **login** свойство `type = "string"`, поэтому `_presets = "string"`, сет берется из `elements.sets.string`.

В поле **password** `_presets` равен `"string:$password"`. Символ `:` разделяет сеты, которые должны быть слиты в один. При рендере поля, будут взяты данные из `elements.sets.string` и из `elements.sets.$password`, а затем объединены в один объект слева направо, т.е. свойства из `elements.sets.$password` перезапишут такие же `elements.sets.string`. Слияние глубокое, все вложенные объекты рекурсивно объединяются.

Сеты бывают осоновными (без ведущего `$` в названии) и вспомогательными (с ведущим `$` в названии). У основного есть свойство `$_widget` с React-элементом, производящим рендер, а у вспомагательного нет, т.к. задача вспомогательного расширять и менять основной сет своими данными. [Ссылка на документацию по сетам.](#)
...
```
`ff_layout` должен быть объектом, в свойстве `$_widget` которого должен быть передан React-элемент, который отрендерит объект. Если `$_widget` не определен, то по-умолчанию используется `div`. Из объекта, помимо `$_widget` изымаются `$_fields` и `$_maps`, а то, что останентся передается как `props` React-элементу определенному в `$_widget`.

Свойство `$_fields` должно быть массивом и оно определят порядок размещения полей и объктов. Его строковые значения заменяются полями из свойства `properties` в случае если `type = "object"` или `items` в случае `type = "array"`. Если поле в схеме не найдено, то оно пропускается. В свою очередь значения-объекты проходят такую же обработку как и `_layout` (рекурсивно).
...
С полями `login` и `password` все понятно, тут определен их порядок, а затем идет объект, который выводит кнопку отправки. Рассмотрим его подробнее. В качестве `$_widget` передан HTML-тэг `button`, в свойствах передан `type` равный `submit` и потомки в свойстве `children`, где передан текст `Send`, т.е. будет выведет следующий HTML-код `<button type="submit">Send</button>`. 
...
Улучшим нашу форму, добавим кнопку ресет и дополнительный, вложенный слой, чтобы отобразить эти кнопки в ряд:
```js
schema = {
    "type": "object",
    "_layout": {
...
```
Теперь на 3-й позиции объект со свойством `$_fields`. Так как `$_widget` у него не определен, то по-умолчанию он равен `div`. Свойство `className` перед тем как быть переданым `_$widget` предварительно обрабатывается функцией `elements._$cx`. Как она работает описано в [документвции](https://github.com/wtnm/fform/blob/master/documentation.md#cx). В `$_fields` опредено 2 объекта (их обработка описана на предыдущем шаге) которые передаются `_$widget` в качестве потомков (childen). В итоге получается следуюзий HTML-код: `<div class="inline"><button type="submit">Send</button><button type="reset">Clear</button></div>`. 
...
Рассмотрим способ расширения объектов `elements`. В каждом объекте можно объявить свойство `$_ref`. Значение этого свойства должна быть одна или несколько ссылок на `elements` вида `^/...` разделенных символом `:`. Объект, которые содержит `$_ref` будет обработан следующим образом: сначала будут собраны все объекты по ссылкам в `$_ref`, а затем все собранные объекты и текущий объект без свойства `$_ref` будут объединены в один. Данный механизм применяется для всего объекта `elements`, а так же для свойств схемы `ff_layout`, `ff_custom`, `ff_validators`, `ff_dataMap`, `ff_oneOfSelector`. 
...
В базового `elements` уже присутствуют часто используемые объекты формы ([подробнее в документации](https://github.com/wtnm/fform/blob/master/documentation.md#parts)). Перепишем `ff_layout` с их использованием `^/parts/Submit` и `^/parts/Reset`:
```js
...
"_layout": {
    "$_fields": [
...
...
```
При обработке `{"$_ref": "^/parts/Submit", "children": ["Send"]}` будет взят объект по ссылке `elements.parts.Submit`, затем он будет слит с объектом `{"children": ["Send"]}}`, а после этого он будет обработан так, как уже было описано на предыдущем шаге. 
...
Добавим маску ввода для поля `login` вида `XXX-XXX-XXXX`. Для этого сначала определим функции, которые будут форматировать и парсить значение поля и добавим их в `elements`. Обратите внимание функции возвращают полученный массив аргументов, изменяя в нем только самый первый. Это связано с особенностями применения [обработчиков данных](https://github.com/wtnm/fform/blob/master/documentation.md#data-processors).
```js
...
elements.extend([{
    fn:{
...
`$_maps` - это объект, который определяет какие свойства пробрасывать из state в компонент, при этом есть возможность использовать [обработчик данных](https://github.com/wtnm/fform/blob/master/documentation.md#data-processors) для их изменения ([подробнее в документации](https://github.com/wtnm/fform/blob/master/documentation.md#_maps)). Используем это, для того чтобы добавить обработчик для свойства `value` (сейчас в свойство компнента просто передается значение из state, без обработки)
```js
schema = {
	,,,
    "properties": {
...
```
Теперь перейдем к свойству `onChange`. В него сейчас назначен следующий обработчик `{"$":^/fn/eventValue|^/fn/setValue`}. Немного разъяснений по его функционированию: Обработчик получает event, который передает функции `elements.fb.eventValue`. Эта функция берет из event'а значение `event.target.value` и передает это значение вместо event'а следующей функции. Все остальные аргументы (если они есть) передаются без изменений. `setValue` использует [API](https://github.com/wtnm/fform/blob/master/documentation.md#api) для того, чтобы изменить state и поменять значение поля на переданное. Добавим между `eventValue` и `setValue` нащ парсер.
```js
schema = {
	,,,
    "properties": {
...
[Текущий результат](#https://wtnm.github.io/fform-constructor/index.html#url=tutorial_adv.json&selector=0).
...
[Текущий результат](#https://wtnm.github.io/fform-constructor/index.html#url=tutorial_adv.json&selector=1).
...
[Итоговый результат](#https://wtnm.github.io/fform-constructor/index.html#url=tutorial_adv.json&selector=1).
